classdef FlatCover < LightStep
    ...
    methods
        ...
        function pattern = GetFarFieldPattern(obj, h, v, previous, ~)
            ...
            % in tan^2 and cos^2 form
            % TODO Angles greater than 90 are trimmed
            tan2Theta = tand(h - obj.HorizontalAngle).^2 + tand(v - obj.VerticalAngle).^2;
            cos2Theta = 1./ (tan2Theta + 1);
            cosTheta = sqrt(cos2Theta);

            % Compute the cos of the internal incidence angle
            cosPhi = sqrt(1 - (1 - cos2Theta)/obj.RefractiveIndex^2);

            % Absorption using exponential decay (Beer-Lambert Law)
            A = exp(-obj.AttenuationCoefficient * obj.Thickness ./ cosPhi);
